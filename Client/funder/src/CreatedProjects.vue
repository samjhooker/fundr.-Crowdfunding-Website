    ...
    export default {
        ...
        methods: {
            getData: function (status) {
                ...
                switch(status){
                    case "all":
                        url = this.$root.$data.url + 'projects?creator=' + localStorage.getItem("currentUserId")
                        break;
                    case 'open':
                        url=this.$root.$data.url + 'projects?open=true&creator=' + localStorage.getItem("currentUserId");
                        break;
                    case 'closed':
                        url = this.$root.$data.url + 'projects?open=false&creator=' + localStorage.getItem("currentUserId");
                        break;
                    default:
                        url = this.$root.$data.url + 'projects?creator=' + localStorage.getItem("currentUserId")
                        break;
